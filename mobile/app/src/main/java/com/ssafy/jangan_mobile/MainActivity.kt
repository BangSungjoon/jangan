...
class MainActivity : ComponentActivity() {
    ...
    // 포그라운드에서만 사용할 비콘 옵저버 (더 빠른 응답을 위해)
    private val foregroundBeaconObserver = Observer<Collection<Beacon>> { beacons ->
        if (!isAppInForeground) return@Observer // 앱이 백그라운드에 있으면 무시
        ...
        if (found) {
            ...
            beacons.forEach { beacon ->
                if (beacon.id1.toString().startsWith("AAAAA204", true)) {
                    ...
                    Log.d("ForegroundBeacon", "포그라운드에서 감지된 비콘 - code:${code} distance:${distance}")
                }
            }
    ...
    // Notification 채널 생성 (Firebase 메시지용)
    private fun createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val channelId = "alert"
            val channelName = "화재 알림"
            val importance = NotificationManager.IMPORTANCE_HIGH
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Firebase 초기화 및 토픽 구독
        FirebaseApp.initializeApp(this)
        Firebase.messaging.subscribeToTopic("alert")
            .addOnCompleteListener { task ->
                var msg = "Firebase 토픽 구독 완료"
                if (!task.isSuccessful) {
                    msg = "Firebase 토픽 구독 실패"
                }
                Log.d(TAG, msg)
            }
